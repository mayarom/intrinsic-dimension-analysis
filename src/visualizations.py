...
# }
#
# PCA_THRESHOLDS: tuple[float, float, float] = (0.90, 0.95, 0.99)
#
# # ── Palette: muted, accessible, publication-friendly ──────────────────────────
# _BLUE   = "#2166AC"
# _RED    = "#D6604D"
# _GREEN  = "#4DAC26"
# _ORANGE = "#F4A582"
# _GREY   = "#636363"
#
# # Three visually distinct, ordered colors for the three PCA thresholds
# THRESHOLD_COLORS: tuple[str, str, str] = (_RED, _ORANGE, _GREY)
# BAR_COLORS: tuple[str, str] = (_BLUE, _RED)
#
#
...
#
#     Uses a LaTeX-compatible serif typeface (Palatino via mathpazo aesthetics),
#     a restrained color cycle, and explicit spine / grid settings suitable for
#     camera-ready academic figures.
#     """
#     plt.rcParams.update({
#         # ── Resolution ──────────────────────────────────────────────────────
#         "figure.dpi":       120,
...
#
#         # ── Color cycle (ColorBrewer RdBu-like, 6 distinct) ─────────────────
#         "axes.prop_cycle": plt.cycler(color=[
#             _BLUE, _RED, _GREEN, _ORANGE, _GREY, "#762A83",
#         ]),
#
#         # ── Lines & markers ─────────────────────────────────────────────────
#         "lines.linewidth":  1.8,
...
#
#
# def _all_thresholds_same_component(threshold_components: dict[float, int]) -> bool:
#     """Return True if all PCA threshold crossings fall on the same component."""
#     values = list(threshold_components.values())
...
#         axes[i].imshow(image, cmap="gray_r", interpolation="nearest")
#         axes[i].set_title(f"Label: {y_mnist[idx]}", fontsize=8.5, pad=3, color="#333333")
#         axes[i].set_xticks([])
#         axes[i].set_yticks([])
#         for spine in axes[i].spines.values():
#             spine.set_visible(False)
#
#     fig.suptitle("Random Sample of MNIST Digit Images", fontsize=13, fontweight="bold", y=1.01)
...
#
#     Annotation strategy:
#     - If all thresholds map to the same component (e.g. 2D Plane, Swiss Roll),
#       a single unified annotation is used instead of three overlapping ones.
#     - For Gaussian Noise (thresholds clustered in a narrow range), only the
#       90% and 99% thresholds are annotated to avoid crowding.
#     - For all other datasets, the 90% and 95% thresholds are annotated;
#       99% is marked with a line and point only.
#     - No fill under the curve.
#     - No legend (the curve's identity is self-evident).
#     """
...
#     collapsed = _all_thresholds_same_component(threshold_components)
#
...
#
#     if collapsed:
#         # All thresholds converge on one component: draw a single vertical line
#         # and one unified annotation instead of three overlapping ones.
#         single_pc = list(threshold_components.values())[0]
#         ax.axvline(single_pc, linestyle="--", linewidth=1.4, color=_GREY, alpha=0.85, zorder=2)
#         ax.scatter([single_pc], [0.99], s=40, color=_GREY, zorder=5,
#                    edgecolors="white", linewidths=0.8)
#         ax.annotate(
#             f"All thresholds (90/95/99%) at PC {single_pc}",
#             xy=(single_pc, 0.95),
#             xytext=(single_pc + max(0.08, 0.04 * (x_zoom_max or len(cumulative_variance))),
#                     0.68),
#             textcoords="data",
#             fontsize=7.5,
#             color=_GREY,
#             ha="left",
#             va="bottom",
#             arrowprops={"arrowstyle": "-", "lw": 0.6, "color": _GREY, "alpha": 0.55},
#         )
#     else:
#         is_gaussian = dataset_name == DATASET_DISPLAY_NAMES["gaussian_noise"]
#
...
#             ax.axvline(component_count, linestyle=":", linewidth=1.0,
#                        color=color, alpha=0.75, zorder=2)
#             ax.scatter([component_count], [threshold], s=36, color=color,
#                        zorder=5, edgecolors="white", linewidths=0.8)
#
#             # Annotation logic:
#             # - Gaussian: only annotate 90% and 99% (skip 95% to reduce clutter)
#             # - Others:   only annotate 90% and 95%; skip 99% text
#             skip_annotation = (
#                 (is_gaussian and threshold == 0.95)
#                 or (not is_gaussian and threshold == 0.99)
#             )
#             if skip_annotation:
#                 continue
#
...
#
#             if is_gaussian and threshold == 0.99:
#                 # Place 99% label well to the left to avoid right-edge crowding
#                 x_text = component_count - 3.5
#                 y_text = threshold - 0.08
#                 ha = "right"
#             elif is_gaussian and threshold == 0.90:
#                 # Push 90% label down slightly so it doesn't crowd 99%
#                 x_text = component_count - 3.5
#                 y_text = threshold - 0.08
#                 ha = "right"
#             else:
#                 x_text = component_count + max(1, int(0.015 * len(cumulative_variance)))
#                 y_text = min(threshold + 0.028, 1.01)
#                 ha = "left"
#
#             if dataset_name == DATASET_DISPLAY_NAMES["mnist"] and threshold == 0.99:
#                 x_text = component_count - 58
...
#                 ha=ha,
#                 va="bottom",
#                 arrowprops={"arrowstyle": "-", "lw": 0.7, "color": color, "alpha": 0.65},
#             )
#
...
#
#     Annotation strategy per dataset:
#     - MNIST:        label PC 1 only (top PCs are very close together)
#     - Gaussian:     no annotations (flat curve, labels add noise)
#     - 2D Plane:     label PC 1 and PC 2 only (PC 3 is near-zero)
#     - Swiss Roll:   no annotations (only 3 points, structure is self-evident)
#     No fill under the curve in any case.
#     """
#     pca_results = get_pca_results(x_processed)
...
#
#     # Per-dataset annotation count
#     _annotation_counts: dict[str, int] = {
#         DATASET_DISPLAY_NAMES["mnist"]:           1,
#         DATASET_DISPLAY_NAMES["gaussian_noise"]:  0,
#         DATASET_DISPLAY_NAMES["plane_2d_in_10d"]: 0,
#         DATASET_DISPLAY_NAMES["swiss_roll"]:      0,
#     }
#     top_k = _annotation_counts.get(dataset_name, 1)
#
#     fig, ax = plt.subplots(figsize=(8.5, 5.2))
#     _style_spine(ax)
#
...
#         zorder=3,
#     )
#     # No fill — avoids visual weight without adding information
#
#     if 0 < top_k <= len(explained_variance_ratio):
#         for i in range(top_k):
#             # Offset downward slightly so label doesn't escape top of axes
#             y_offset = -0.015 if explained_variance_ratio[i] > 0.85 else 0.012
#             va = "top" if y_offset < 0 else "bottom"
#             ax.annotate(
#                 f"PC {i + 1}",
#                 xy=(components[i], explained_variance_ratio[i]),
#                 xytext=(components[i] + 0.8, explained_variance_ratio[i] + y_offset),
#                 fontsize=8.5,
#                 color=_GREY,
#                 ha="left",
#                 va=va,
#                 arrowprops={"arrowstyle": "-", "lw": 0.7, "alpha": 0.55},
#             )
#
...
#
#     Visual approach:
#     - The scaling region is distinguished by color and marker weight only.
#     - No axvspan shading — it competed with the data.
#     - No redundant "Scaling region" text annotation — the legend suffices.
#     - Compact legend placed upper-left.
#     """
...
#     _style_spine(ax)
#
#     # Full log-log curve — slightly stronger grey so it reads in print
#     ax.plot(
#         log_radii,
...
#         zorder=2,
#     )
#     # Scaling region: same markers but heavier and colored
#     ax.plot(
#         log_radii_region,
...
#         zorder=3,
#     )
#     # Linear fit: dashed red, legend label contains the estimated dimension
#     ax.plot(
#         log_radii_region,
#         fitted_line,
#         linestyle="--",
...
#         zorder=4,
#     )
#     # No axvspan, no text annotation
#
#     ax.set_xlabel(r"$\log(r)$", labelpad=8)
#     ax.set_ylabel(r"$\log\,C(r)$", labelpad=8)
#     ax.set_title(
#         f"Correlation Dimension – Log–Log Plot\n{dataset_name}",
#         fontsize=12,
#         fontweight="bold",
#         pad=10,
#     )
#     ax.legend(loc="upper left", frameon=True, fontsize=8, borderpad=0.35, labelspacing=0.25)
#     fig.tight_layout()
#     save_figure(output_filename)
#
...
# ) -> None:
#     """
#     Plot a grouped bar chart comparing PCA and correlation-dimension estimates.
#     """
...
#     ax.set_xticklabels(dataset_names, rotation=10, ha="right", fontsize=9.5)
#     ax.set_ylabel(
#         "Estimated Dimension (log scale)" if use_log_scale else "Estimated Dimension",
#         labelpad=8,
#     )
...
#     if use_log_scale:
#         ax.set_yscale("log")
#         ax.set_ylim(top=max(float(max(pca_values)), max(correlation_values)) * 3.0)
#         add_bar_labels(
#             ax=ax,
#             x_positions=x_positions - width / 2,
#             values=[float(v) for v in pca_values],
#             fmt="{:.0f}",
#             multiplicative_offset=1.18,
#         )
#         add_bar_labels(
#             ax=ax,
#             x_positions=x_positions + width / 2,
...
#         )
#     else:
#         max_val = max(float(max(pca_values)), max(correlation_values))
#         ax.set_ylim(top=max_val * 1.28)
#         add_bar_labels(
#             ax=ax,
#             x_positions=x_positions - width / 2,
#             values=[float(v) for v in pca_values],
#             fmt="{:.0f}",
#             additive_offset=max_val * 0.025,
#         )
#         add_bar_labels(
#             ax=ax,
#             x_positions=x_positions + width / 2,
...
#
#
# def plot_dimension_comparison_logscale(
#     pca_summary: dict[str, int],
#     correlation_summary: dict[str, float],
# ) -> None:
#     """
#     Plot a comparison between PCA-based and correlation dimension estimates
#     using a logarithmic y-axis for readability.
#     """
...
#         title="Comparison of PCA and Correlation Dimension Estimates",
#         use_log_scale=True,
#     )
#
#
# def plot_dimension_comparison_synthetic_only(
#     pca_summary: dict[str, int],
#     correlation_summary: dict[str, float],
# ) -> None:
#     """
#     Plot a comparison figure for the synthetic datasets only, using a linear scale.
#     """
#     synthetic_names = [
...
#         title="Comparison of PCA and Correlation Dimension – Synthetic Datasets",
#         use_log_scale=False,
#     )
#
#
...
#     }
#
#     plot_dimension_comparison_logscale(
#         pca_summary=pca_summary,
#         correlation_summary=correlation_summary,
#     )
#     plot_dimension_comparison_synthetic_only(
#         pca_summary=pca_summary,
#         correlation_summary=correlation_summary,
#     )
#
